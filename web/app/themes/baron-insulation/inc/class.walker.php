...
class wp_bootstrap_navwalker extends Walker_Nav_Menu
{
	...
	public function start_lvl(&$output, $depth = 0, $args = array())
	{
		...
		if ($depth == 0 && $this->menu_grid == 'menu-grid') {
			$output .= "\n$indent<div class=\"has-submenu\"><div class=\"submenu\">\n";
			...
			if ($depth > 0) {
				$output .= "\n$indent<div  class=\"submenu\"><ul role=\"menu\">\n";
			} else {
				$output .= "\n$indent<div  class=\"submenu\"><ul role=\"menu\">\n";
			}
		}
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		if (strcasecmp($item->attr_title, 'divider') == 0 && $depth === 1) {
			...
			$baron_class = apply_filters('nav_menu_css_class', array_filter($classes), $item, $args);

			$class_names = join(' ', apply_filters('nav_menu_css_class', array_filter($classes), $item, $args));
			if ($args->has_children) {
				$class_names .= ' dropdown child-dropdown has-submenu';
			}
			...
			if ($args->has_children && !in_array('tab-heading', $classes)) {
				...
				$atts['class']          = 'dropdown-toggle baronActiveLink';
			} else {
				$atts['href'] = !empty($item->url) ? $item->url : '';
			}
			...
			if (in_array('heading', $classes)) {
				...
				$output .= $indent . '<p class="submenu__title">' . $item->title . '</p>' . '<p>' . $item->description . '</p>'. '<p class="color-theme">' . $item->attr_title . '</p>';
				
								
			} elseif (in_array('cta-type-one', $classes)) {
			
				$this->heading = 'cta-type-one';
				$output .= $indent . '<div class="item links"><a class="link-item bgcolor-lightblue-alt" href="'. $item->url .'"><h6 class="title">' . $item->title . '</h6>' . '<p>' . $item->description . '</p></a>';
									
			} elseif (in_array('cta-type-two', $classes)) {
			
				$this->heading = 'cta-type-two';
				$output .= $indent . '<a class="link-item bgcolor-blue-alt" href="'. $item->url .'"><h6 class="title">' . $item->title . '</h6>' . '<p>' . $item->description . '</p></a>';
	...
	public static function fallback($args)
	{
		if (current_user_can('manage_options')) {
			...
			if ($container)
				$fb_output .= '</' . $container . '>';
			echo sprintf(__('%s', 'baron'), $fb_output);
		}
	}
}